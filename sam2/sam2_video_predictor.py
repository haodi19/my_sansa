...
class SAM2VideoPredictor(SAM2Base):
    ...
    def add_new_mask_batch(
        ...
        # direct output with mask
        pix_feat = feats[-1].permute(1, 2, 0)
        pix_feat = pix_feat.view(-1, self.hidden_dim, *sizes[-1])
        sam_outputs = self._use_mask_as_output(
            pix_feat, high_res_features, mask
        )
        (
            _,
            _,
        ...
        return low_res_masks, obj_ptr, maskmem_features, maskmem_pos_enc
    
    # ========================================
    # Add mask prompt for refinement
    # Batch version
    # ========================================
    def mask_refinement_batch(
        self,
        feats,
        sizes,
        mask,
    ...
    def propagate_in_video_batch(
        ...
        # fuse input feats with memory features
        num_obj_ptr_tokens = 0
        to_cat_memory, to_cat_memory_pos_embed = [], []
        selected_cond_outputs = {  # only support 1-shot now!!!!!!
            0: {
                "maskmem_features": mem_feats,
                "maskmem_pos_enc": mem_poss,
                "pred_masks": preds,
                "obj_ptr": obj_ptr,
            }
        }
        ...
        for t_pos, prev in t_pos_and_prevs:
            if prev is None:
                continue
            to_cat_memory.append(prev["maskmem_features"].cuda(non_blocking=True).flatten(2).permute(2, 0, 1))
        ...
        if self.use_obj_ptrs_in_encoder:
            ...
            pos_and_ptrs = [
                (1, out["obj_ptr"])  # 1 is only for the first frame now!!!!!!
    ...
    # Batch version (final)
    # ========================================
    def propagate_in_video_batch_mine(
        self,
        feats,          # b, c, h, w - qry feat
        poss,
        sizes,
        sup_mem_feats,  # b*s, c, h, w - sup mem feat
        sup_mem_poss,
        sup_preds,
        sup_obj_ptr,
        prior=None,
        ...
        sup_mem_feats = sup_mem_feats.view(B, shot, -1, H, W)
        sup_mem_poss[-1] = sup_mem_poss[-1].view(B, shot, -1, H, W)
        sup_preds = sup_preds.view(B, shot, -1, H, W)
        sup_obj_ptr = sup_obj_ptr.view(B, shot, -1)
        # import pdb
        # pdb.set_trace()
        for idx in range(shot):
            selected_cond_outputs[idx] = {
                "maskmem_features": sup_mem_feats[:, idx, ...],
                "maskmem_pos_enc": [sup_mem_poss[-1][:, idx, ...]],
                "pred_masks": sup_preds[:, idx, ...],
                "obj_ptr": sup_obj_ptr[:, idx, ...]
            }

        t_pos_and_prevs = [(0, out) for out in selected_cond_outputs.values()]
        # t_pos_and_prevs = [(self.num_maskmem-1, out) for out in selected_cond_outputs.values()]
        # t_pos_and_prevs = [(1, out) for out in selected_cond_outputs.values()]
        ...
        if self.use_obj_ptrs_in_encoder:
            ...
            if len(pos_and_ptrs) > 0:
                ...
                #     max_obj_ptrs_in_encoder = min(2, self.max_obj_ptrs_in_encoder)
    ...
    # Batch version (final)
    # ========================================
    def propagate_in_video_batch_final(
        self,
        feats,          # b, c, h, w - qry feat
        poss,
        sizes,
        sup_mem_feats,  # b*s, c, h, w - sup mem feat
        sup_mem_poss,
        sup_mem_masks,  # b*s, c, h, w - sup mem mask
        sup_preds,
        sup_obj_ptr,
        qry_mem_feats,  # b, c, h, w - qry mem feat
        qry_mem_poss,
        qry_preds,
        qry_obj_ptr,
        prior=None,
        ...
        selected_cond_outputs[shot] = {
            "maskmem_features": qry_mem_feats,
            "maskmem_pos_enc": qry_mem_poss,
            "pred_masks": qry_preds,
            "obj_ptr": qry_obj_ptr,
        }
        ...
        # memory attention
        pix_feat_with_mem, weights = self.memory_attention(
            curr=feats[-1:],
            curr_pos=poss[-1:],
            memory=memory,
            memory_pos=memory_pos_embed,
            memory_mask=sup_mem_masks,
            num_obj_ptr_tokens=num_obj_ptr_tokens,
            calibrate=True
        )
        pix_feat_with_mem = pix_feat_with_mem.permute(1, 2, 0).view(B, C, H, W)
    ...
    # Batch version
    # ========================================
    def get_image_feature_batch(self, image):
